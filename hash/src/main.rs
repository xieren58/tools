...
fn build_app() -> Command<'static> {
    Command::new("hash")
        ...
        .arg(Arg::new("sha256").short('S').long("sha256").help("Compute the hash using sha256 algorithm (Default)."))
        .arg(Arg::new("md5").short('M').long("md5").help("Compute the hash using md5 algorithm."))
        .arg(Arg::new("blake3").short('B').long("blake3").help("Compute the hash using blake3 algorithm."))
        .arg(
            Arg::new("text")
                .short('t')
                .long("text")
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match *self {
            HexError::InvalidHexPrefix { hex } => {
                write!(f, "Invalid hex prefix {}, should be 0x or 0X", hex)
            }
...
fn hex_to_byte(hex_string: &str) -> Result<u8, HexError> {
    ...
    let hi = val(chars.next().unwrap(), hex_string)?;
    let lo = val(chars.next().unwrap(), hex_string)?;
...
fn compute(matches: &ArgMatches, inputs: &[HashInput]) {
    ...
    for input in inputs.iter() {
        ...
        let input_bytes = match input {
            ...
            HashInput::FILE(file) => {
                ...
                if hex_input {
                    hex_to_byte_slice(&std::fs::read_to_string(file).unwrap())
                } else {
